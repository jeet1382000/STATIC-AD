...
SEED_TEMPLATES: List[dict] = [
    ...
    {"number": 5, "name": "Us vs Them Comparison", "aspect": "1:1", "needs_product": True, "category": "comparison", "enabled": True,
     "scaffold": "Side-by-side comparison ad for [BRAND NAME] in a strict 50/50 vertical split. LEFT half (the brand half): the uploaded product hero-staged on the brand's signature background, beautifully lit, sharp; column header at the top in tracked caps reads '[BRAND NAME]' set in the brand display type and brand primary color. Four checklist rows underneath, each with a small filled circle in the brand accent and a confident benefit in 2–3 words (examples to seed Claude: 'SILENT', 'BUILT TO LAST', 'STREAMLINED', 'PREMIUM LEATHER'). RIGHT half: a muted, slightly-desaturated neutral grey field; column header reads 'THE ALTERNATIVE' (or 'GENERIC') in greyscale; instead of a product show a tasteful abstract grey block silhouette OR a chaotic photographic suggestion of the pain point (e.g. tangled cables, cluttered desk, jingling keychain). Same four labels but rendered in 50% grey with thin grey strike-throughs or hollow circles. Thin vertical hairline divides the two halves. Swiss grid, no decorative noise."},
